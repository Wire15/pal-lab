...
// === pal-save vendored entrypoint (powzix/ooz) ==============================
// Upstream `main` CLI, the oo2core DLL loader, and the verify/load_file helpers
// were removed for the vendored build: this translation unit provides
// decompression only and must not define `main` (it links into a Rust binary).
//
// SAFE_SPACE: the Kraken decompressor may write up to 64 bytes past `dst_len`
// as scratch. The Rust caller over-allocates the destination buffer by at least
// this many bytes; see `oodle_decompress` in src/compress.rs.
