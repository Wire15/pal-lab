...
### Own-install extraction (2026-07-21)

Dumped game data straight from a legitimately-owned Palworld 1.0 install (build **24181527**) with the `tools/pal-extract` utility (.NET 10, **CUE4Parse**, `PalworldModding/UsefulFiles` usmap). Output vendored to `crates/pal-data/vendor/extracted-game-data.json` and merged into the pack at build time.

- **Elements are now ground truth.** Per-species `ElementType1/2` from `DT_PalMonsterParameter` replaces the former derived `vendor/elements.json` (deleted). Diffing old-vs-new across all 299 pack species produced **zero** element changes — the extraction confirms the 5 corrections previously applied by hand.
- **Partner-skill descriptions are NOT in the game files.** The extraction yields a partner-skill *name* for every species (299/299), but every `description` field is `null` — Palworld ships partner-skill flavor text only in localization/UI assets, not the data tables. So names now come from the extraction (authoritative) while the 169 authored *descriptions* stay sourced from `vendor/partner-skills.json` (paldex-api), merged by internal name.
- **Extended stats** (price, craft/slow-walk/walk/run/ride-sprint/transport speeds, stamina, max-full-stomach, size class) added to the pack + paldex payload. `ride_sprint_speed`/`transport_speed` carry a `-1` sentinel for non-rideable / non-transport species (preserved verbatim; the UI renders a dash).
- **IV-inheritance conflict, unresolved deliberately.** The extracted `Combi_TalentInheritNum` CDO array is **[3, 2, 1]**, which normalizes to ~50/33/17% for 1/2/3 IVs inherited. This *conflicts* with palcalc's empirically-sampled **50/25/25** ([2, 1, 1]) model that our solver and oracle fixtures validate against. Decision: **keep the empirical [2, 1, 1] weights in the solver** (`InheritanceWeights::default`, unchanged); store the CDO **[3, 2, 1]** in the pack as `game_settings.combi_talent_inherit_num` **data only**. Wiring the CDO arrays into the solver at runtime is future work — the earlier "→ Extract the CDO defaults to settle it" note (Mechanics ground truth §) is now answered: the CDO exists and is captured, but the empirical distribution wins until re-sampled against the current build. `Combi_PassiveInheritNum` / `Combi_PassiveRandomAddNum` both **[4, 3, 2, 1]** agree with the shipped solver weights; `Combi_BossPalRate` = 0.05.
- **Reconciliation.** Extraction carries 305 species vs our 299-species pack. The 6 extras — `BlackFurDragon`, `DarkMutant`, `ElecSnail_Fire`, `Monkey_Ice`, `POLICE_HawkBird`, `POLICE_ThunderDog` — are cut / NPC-only content with no icons or breeding data and are intentionally dropped (not playable pals we're missing). Zero pack species are absent from the extraction. Spot-checking db.json's kept fields (rarity, food_amount, combi_rank, nocturnal, male_probability) against the extraction found **zero mismatches**, so hp/attack/defense/rarity/food/combi/nocturnal stay sourced from db.json as before.
