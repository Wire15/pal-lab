...
// The active-names fixture (`active-names.json`) is generated by the DataTier
// slice and may not exist yet. A glob with a zero-match pattern compiles to an
// empty map, so a missing file degrades to `{}` (the humanizer fallback in
// `lib/active-names.ts` then takes over) instead of breaking the dev build.
const activeNamesGlob = import.meta.glob<Record<string, string>>(
  "../dev-fixtures/active-names.json",
  { import: "default" },
);
...
async function invokeDev<T>(cmd: string, args?: InvokeArgs): Promise<T> {
  ...
  if (cmd === "list_active_names") {
    const loader = activeNamesGlob["../dev-fixtures/active-names.json"];
